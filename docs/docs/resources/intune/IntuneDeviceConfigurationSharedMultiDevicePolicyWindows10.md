...
| **FastFirstSignIn** | Write | String | Specifies whether to auto connect new non-admin Entra ID accounts to pre-configured candidate local accounts. Possible values are: notConfigured, enabled, disabled. | `notConfigured`, `enabled`, `disabled` |
